...
PHASES = {
    "Phase 1": {
        ...
        2025,
        "description":
        "Fast Kickstart — immediate execution, fastest scaling, early cash inflow",
        "target_by":
        "Dec 2025",
        "start_date":
        date(2025, 1, 1),
...
PHASE_STATE = {
    name: {
        "state":
        "Pending" if date.today() < meta["start_date"] else "Active",
        "started_on":
        None if date.today() < meta["start_date"] else str(date.today()),
    }
    ...
    for pname, pdata in PHASES.items():
        for s in pdata["streams"]:
            ...
            try:
                ...
                logging.warning(
                    "Stream %s failed: %s — attempting to regen connector",
                    s[0], e)
...
# -----------------------
def generate_pdf_summary(total,
                         details,
                         outpath="DailySummary.pdf",
                         passcode=PDF_PASS):
...
def gmail_autoreply_once():
    ...
    try:
        ...
        for mid in mail_ids:
            ...
            # send reply
            reply_text = (
                "Hello — this is an automated JRAVIS reply. "
...
def phases_route():
    ...
    for pname, pdata in PHASES.items():
        ...
        out[pname] = {
            "state": PHASE_STATE[pname]["state"],
            "start_date": str(pdata["start_date"]),
            "monthly_min": mn,
            "monthly_max": mx,
            "streams": details
        }
...
def generate_report():
    ...
    if pdf:
        send_internal_email("JRAVIS Daily Summary",
                            f"Attached summary (total ₹{total})",
                            attachments=[pdf])
        return jsonify({"ok": True, "file": pdf})
    else:
        return jsonify({
            "ok": False,
            "msg": "pdf not available, but report data generated",
            "total": total,
            "details": details
        })
    ...
    if openai and OPENAI_KEY:
        try:
            res = openai.ChatCompletion.create(
                model="gpt-4o-mini"
                if hasattr(openai, "ChatCompletion") else "gpt-3.5-turbo",
                messages=[{
                    "role":
                    "system",
                    "content":
                    "You are JRAVIS, an assistant for the Boss."
                }, {
                    "role": "user",
                    "content": msg
                }],
            ...
            if isinstance(res, dict) and "choices" in res and len(
                    res["choices"]) > 0:
    ...
    if "when" in low and "phase" in low:
        for pname in PHASES:
            if pname.lower() in low:
                return jsonify({
                    "reply":
                    f"{pname} starts on {PHASES[pname]['start_date']} (state: {PHASE_STATE[pname]['state']})."
                })
        lines = [
            f"{p}: {PHASES[p]['start_date']} ({PHASE_STATE[p]['state']})"
            for p in PHASES
        ]
...
def check_and_start_phases():
    today = date.today()
    for pname, pdata in PHASES.items():
        if PHASE_STATE[pname]["state"] == "Pending" and today >= pdata[
                "start_date"]:
...
def run_daily_job():
    ...
    total, details = run_streams_once()
    pdf = generate_pdf_summary(
        total,
        details,
        outpath=f"daily_{datetime.utcnow().strftime('%Y%m%d')}.pdf")
    body = f"JRAVIS Daily Snapshot — total (simulated): ₹{total}"
    try:
        if pdf:
            send_internal_email("JRAVIS Daily Snapshot",
                                body,
                                attachments=[pdf])
        else:
...
def watchdog_loop():
    ...
    while True:
        ...
        if fail_count >= 12:
            logging.error(
                "Watchdog: persistent failures detected; waiting for platform to restart"
            )
            fail_count = 0
        time.sleep(60)
        ...
        if conn.laddr and getattr(conn.laddr, "port", None) == port:
            ...
            if pid and pid != os.getpid():
                try:
                    ...
                    logging.info("Killing PID %s (owner %s)", pid,
                                 p.username())
                    p.kill()
                except Exception as e:
...
# --- simple fallback data for the dashboard ---
MISSION_TARGET = 1000000
CURRENT_EARNINGS_INR = 250000
CURRENT_EARNINGS_USD = 3000
PHASES = {
    "Phase 1": {
        "streams": [{
...
def main():
    ...
    formatted_inr = f"{CURRENT_EARNINGS_INR:,}"
    formatted_usd = f"{CURRENT_EARNINGS_USD:,.0f}"

    return render_template_string(MAIN_HTML,
                                  earn_inr_fmt=formatted_inr,
                                  earn_usd_fmt=formatted_usd,
...
if __name__ == "__main__":
    logging.info(
        "✅ JRAVIS initializing — scheduler & dashboard (Phases automation enabled)."
    )
