...
class PrintifyConfig:
    ...
    def from_env(cls) -> "PrintifyConfig":
        ...
        return cls(
            ...
            user_agent=os.getenv("PRINTIFY_USER_AGENT",
                                 "printify-connector-v3/1.0"),
        )
...
class PrintifyClient:
    ...
    def _request(
        ...
        for attempt in range(1, self.cfg.max_retries + 1):
            try:
                ...
                logger.debug("HTTP %d for %s %s", resp.status_code, method,
                             url)
                ...
                retry_after = resp.headers.get(
                    "Retry-After") if 'resp' in locals() else None
                if retry_after:
                    try:
                    ...
                    wait = self.cfg.backoff_factor * attempt
                logger.warning(
                    "Rate limited or server error. Sleeping %.1f seconds (attempt %d)",
                    wait, attempt)
                time.sleep(wait)
                continue
                ...
                wait = self.cfg.backoff_factor * attempt
                logger.warning(
                    "Network error: %s. Retrying in %.1f seconds (attempt %d)",
                    e, wait, attempt)
                time.sleep(wait)
                continue
        ...
        # exhausted retries
        raise PrintifyError(
            f"Request failed after {self.cfg.max_retries} attempts"
        ) from last_exc
    ...
    def list_orders(self,
        ...
        if not shop_id:
            raise ValueError(
                "shop_id required (or set default_shop_id in config)")
        ...
        while True:
            ...
            orders = resp.get("data") if isinstance(
                resp, dict) and "data" in resp else (resp or [])
            if not orders:
                break
            for o in orders:
                yield o
    ...
    # Products
    def create_product(self, shop_id: str,
    ...
    def update_product(self, shop_id: str, product_id: str,
                       payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def upload_image(self,
        ...
        with open(image_path, "rb") as f:
            ...
            return self._request("POST",
                                 f"/shops/{shop_id}/uploads/images.json",
                                 files=files)

    # Print Providers
    def get_print_providers(
            self,
            product_blueprint_id: Optional[str] = None) -> Dict[str, Any]:
        """
...
def example_run():
    ...
    shop_id = cfg.default_shop_id or (shops[0].get("id") if shops
                                      and isinstance(shops, list) else None)
    if not shop_id:
        logger.error(
            "No shop_id available. Set PRINTIFY_SHOP_ID or ensure token has shops."
        )
        return
    ...
    try:
        ...
        for idx, order in enumerate(
                client.list_orders(shop_id=shop_id, limit=20)):
            logger.info("Order #%d: id=%s status=%s", idx + 1, order.get("id"),
                        order.get("status"))
            if idx >= 4:
                break
    except Exception as e:
    ...
    try:
        ...
        logger.info("Found %d print providers",
