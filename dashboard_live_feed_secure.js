...
if (!MASTER_KEY) {
  console.error('ERROR: MASTER_KEY not set in .env. Add MASTER_KEY=<very_random_value>');
  process.exit(1);
}
...
if (WRITE_TO_DB) {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS live_feed (id INTEGER PRIMARY KEY AUTOINCREMENT, ts TEXT, type TEXT, payload TEXT)`);
}
...
app.post('/push', (req, res) => {
  ...
  try {
    if (encrypted) {
      ...
      if (!cipher || !iv || !tag) {
        return res.status(400).json({ ok: false, message: 'missing encryption fields' });
      }
  ...
  if (WRITE_TO_DB && db) {
    const stmt = db.prepare(`INSERT INTO live_feed (ts, type, payload) VALUES (?, ?, ?)`);
...
server.listen(PORT, () => console.log(`Secure Live feed server running on port ${PORT}`));
