...
app = Flask(__name__)
app.secret_key = os.getenv("FLASK_SECRET_KEY", "change_this_secret")   # Needed for sessions
...
def unlock():
    if request.method == "POST":
        ...
        return render_template_string("<h3>Wrong code</h3><a href='/unlock'>Try again</a>")
...
@app.before_request
def check_lock():
    if request.endpoint not in ("unlock", "static") and not session.get("unlocked"):
...
def dashboard():
    ...
    """
    return render_template_string(html, orders=total_orders, revenue=total_revenue, data=data)
...
def api_summary():
    ...
    return jsonify({
        ...
        "daily_summary": [
            {"day": d, "orders": o, "revenue": r} for (d, o, r) in data
        ]
    })
